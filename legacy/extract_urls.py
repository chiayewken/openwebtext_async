...
if args.single_file is None:
    ...
    print("Processing the following files:", filenames, "\n")
else:
    ...
    with decompress(path, "r") as psfile:
        ...
        with open(out_path, "w") as outfile:
            ...
            for line in psfile:
                ...
                if (
                    (not is_bad_url(j["url"]))
                    and (j["score"] > args.min_karma - 1)
                    and (not j["over_18"])
                ):
                    ...
                    if hit_count % 10000 == 0:
                        ...
                        print(
                            "-- Links saved ({}), Links processed ({}), Percent saved ({}%), Errors ({})".format(
                                hit_count, total_count, int(percent_saved), error_count
                            )
                        )
